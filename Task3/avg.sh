...
function calculate1AVG()
{
   ...
   if [ $count -ne 0 ]; then
    ...
    echo "$average $TIMESTAMP" >> /flask_blog/templates/readings/cAVG.txt
...
function calculate2AVG() {
  ...
  for ff in $files; do
   ...
   files=$(ls -1t | grep "$1" | tail -n 5)
    ...
    echo "$freeavg $usedavg $TIMESTAMP" $ >> /flask_blog/templates/readings/MAVG.txt
...
function calculate3AVG()
{
  ...
  for disk in "${!free_sum[@]}"; do 
   ...
   echo "$disk $avg_free $avg_available $TIMESTAMP" >> /flask_blog/templates/readings/dAVG.txt
...
cat << EOF > /flask_blog/templates/memory.html
...
cat << EOF > /flask_blog/templates/disk.html
